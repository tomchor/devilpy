...
def parse_dims(name):
    """
    Parses the dimensions of a given variable based on the names

    Expected names are like: w_xy,
    which would return ["X", "Y"]
    """
    dims = list(name.upper().split('_')[-1])
    return dims


def get_grid_centers(*args):
    outs = []
    for X_f in args:
        X_c = (X_f[:-1] + X_f[1:])/2
        outs.append(X_c)
